...
static size_t str_cat ( char * dst, const char * src, const size_t size )
{
  ...
  if ( 0 == src_n ) { return dst_n + strlen ( src ) ; }
...
static int write_utmp ( struct utmp * const utp )
{
  ...
  if ( 0 > i ) {
    ...
    if ( ( RUN_LVL == utp -> ut_type ) || ( BOOT_TIME == utp -> ut_type ) )
    {
      ...
      (void) strncpy ( utp -> ut_id, "~~", sizeof ( utp -> ut_id ) ) ;
      (void) strncpy ( utp -> ut_line, "~", sizeof ( utp -> ut_line ) ) ;

      if ( 0 == uname ( & uts ) ) {
        (void) strncpy ( utp -> ut_host, uts . release,
          sizeof ( utp -> ut_host ) ) ;
      }
      ...
      while ( NULL != ( utp2 = getutent () ) ) {
        if ( NULL != utp2 && utp2 -> ut_pid == utp -> ut_pid ) {
          (void) strncpy ( utp -> ut_line, utp2 -> ut_line,
            sizeof ( utp -> ut_line ) ) ;
...
static int utmp_set_boot ( void )
{
  ...
  ut . ut_pid = 0 ;
  (void) strncpy ( ut . ut_user, "reboot", sizeof ( ut . ut_user ) ) ;
...
static int utmp_set_halt ( void )
{
  ...
  ut . ut_type = RUN_LVL ;
  ut . ut_pid = 0 ;
  (void) strncpy ( ut . ut_user, "shutdown", sizeof ( ut . ut_user ) ) ;
  return write_utmp ( & ut ) ;
}
...
static int utmp_set_runlevel ( const int now, const int prev )
{
  ...
  ut . ut_type = RUN_LVL ;
  ut . ut_pid = n + 256 * p ;
  (void) strncpy ( ut . ut_user, "runlevel", sizeof ( ut . ut_user ) ) ;
  return write_utmp ( & ut ) ;
}
